...
async function renderParallel(token,tools){
  ...
  $('#compareOne').onchange=e=>{state.compare[0]=e.target.value;localStorage.setItem('compareTranslations',JSON.stringify(state.compare));renderBible(++state.loadingToken)};
  $('#compareTwo').onchange=e=>{state.compare[1]=e.target.value;localStorage.setItem('compareTranslations',JSON.stringify(state.compare));renderBible(++state.loadingToken)};
  wireBible();
}
...
function saveNote(){const key=refKey();const notes=JSON.parse(localStorage.getItem('notes')||'{}');const val=$('#noteInput').value.trim();if(val)notes[key]=val;else delete notes[key];localStorage.setItem('notes',JSON.stringify(notes));$('#verseDialog').close();if(state.view==='library')render()}
function toggleBookmark(){const key=refKey();let a=JSON.parse(localStorage.getItem('bookmarks')||'[]');a=a.includes(key)?a.filter(x=>x!==key):[...a,key];localStorage.setItem('bookmarks',JSON.stringify(a));$('#bookmarkBtn').textContent=a.includes(key)?'★ Bookmarked':'☆ Bookmark'}
function setHighlight(color){const key=refKey();const h=JSON.parse(localStorage.getItem('highlights')||'{}');if(color)h[key]=color;else delete h[key];localStorage.setItem('highlights',JSON.stringify(h));$('#verseDialog').close();if(state.view==='bible')render()}
...
async function renderLibrary(){
  ...
  content.innerHTML=sectionHead('My Library','Notes, bookmarks, highlights, reading history, and backups stay under your control.')+`
  ...
  $$('[data-lib-ref]').forEach(b=>b.onclick=()=>goPipeRef(b.dataset.libRef));$('#exportBackupBtn').onclick=exportBackup;$('#importBackupBtn').onclick=importBackup;
}
...
translationSelect.onchange=()=>{const next=translationSelect.value;if(!translationUnlocked(next)){syncTranslations();return}state.translation=next;saveReading();render()};
...
$('#fontBtn').onclick=()=>{state.fontSize=state.fontSize==='normal'?'large':state.fontSize==='large'?'small':'normal';localStorage.setItem('fontSize',state.fontSize);document.body.dataset.font=state.fontSize;toast(`Reading size: ${state.fontSize}`)};
